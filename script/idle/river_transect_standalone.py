...
def get_featureclass_name(workspace, filename):
    ...
    try:
        ...
        print("error in get_featureclass_name - %s" % err.message)
    else:
    ...
    while row:
        ...
        if wid_value not in widlist:
            if interval_profile > row.shape.length:
                print("skip watershed ID %i - too small for river profile interval" % wid_value)
            else:
...
def remove_duplicate_side(crosslines, riverline, radius, sidefield):
    ...
    while row:
        ...
        if m == n and q == p:
            #print "OID: %i, NEAR_DIST: %1.5f" % (row.getvalue(gp.describe(crosslines).oidfieldname), row.getvalue("NEAR_DIST"))
            rows.deleterow(row)
        m = n
        p = q
        row = rows.next()
...
def check_nodatavalue(elevationpoints, fieldname="RASTERVALU"):
    ...
    while row:
        ...
        if rasterv <= -9999:
            print("WARNING: some elevation points have nodata values")
...
def create_vfsegment_field(tlooklines):
    ...
    while row:
        ...
        #print("side: %s, distab: %i" % (side, distab))
...
def measure_midlinetoriver(tlooklines, riverline, watershedmidpoints):
    ...
    print("distclass field added and updated ...")
    ...
    if fcs_list:
        ...
        for fc in fcs_list:
            try:
                ...
                print "cannot add path to list - %s" % err.message
            else:
...
def delete_gisdata(list_data_to_retain=[], workspace=None):
    ...
    if workspace:
        origws = gp.workspace  # get current workspace
        gp.workspace = workspace
        #print "user-defined workspace:", gp.workspace
        ...
        for retain in list_data_to_retain:
            try:
                ...
                if retainpath:
                    ...
                    #print "retainpath is empty"
                    raise ValueError("no catalogpath returned")
        list_data_to_retain = orig_fcs
        #print "list_data_to_retain:", list_data_to_retain
    else:
        pass
        #print "WARNING: all feature classes, tables, and raster datasets will be removed from", gp.workspace
    final_fcs = get_fcs_list(workspace, wildcard=None)
    #print "final_fcs:", final_fcs, "to_retain:", list_data_to_retain
    for fc in final_fcs:
        try:
            if fc not in list_data_to_retain:
                gp.delete_management(fc)
        except:
            print "cannot delete", fc
    if workspace:
        gp.workspace = origws  # return to current workspace
        #print "current workspace:", gp.workspace
...
def main(workspace, in_raster, watershedbound, watershedfid, widlist, interval, interval_profile, count_intervalpr, adjust_edge, count_interval):
    ...
    for wid in widlist:
        ...
        print "processing watershed ID %i (%i of %i) ..." % (wid, widcount, totwid)
        ...
        try:
            ...
            print "%s - skip watershed ID %i" % (err.message, wid)
    ...
    print "processing time: %i minutes" % round(elaptime/60)
...
if __name__ == '__main__':
    try:
        ...
        if adjust_edge.upper() == "TRUE":
            adjust_edge = True
            print "WARNING: reduced inputs by 2 cells to account for edge effects"
        else:
            adjust_edge = False
